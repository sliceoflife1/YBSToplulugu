...
import { type CommentInput } from "@/lib/validations/community";
import { createComment, editComment, deleteComment } from "@/app/community/actions";
...
export default function CommentSection({
  ...
  const onSaveEditComment = async (commentId: string) => {
    ...
    const result = await editComment(commentId, editCommentContent, slug, postId);
    setLoadingId(null);
  ...
  const onDeleteComment = async (commentId: string) => {
    ...
    setLoadingId(commentId);
    const result = await deleteComment(commentId, postId, slug);
    setLoadingId(null);
  ...
  return (
    <div className="mt-8 rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-6 shadow-sm animate-fade-in">
